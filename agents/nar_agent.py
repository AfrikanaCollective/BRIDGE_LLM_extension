...
class NARAgent:
    ...
    def __init__(self, page_number: int = 1):
        """
        Initialize NAR Agent.

        Args:
            page_number: NAR page number (1 or 2)
        """
        if page_number not in [1, 2]:
            raise ValueError(f"NAR only supports pages 1 and 2, got {page_number}")

        self.tools = NARTools(page_number=page_number)
        self.page_number = page_number
        self.schema = get_form_schema('NAR', page_number)
        logger.info(f"🚀 NAR Agent initialized (Page {page_number})")
        logger.info(f"📋 Loaded NAR Page {page_number} Schema with {len(self.schema)} field definitions")

    async def process_nar_file(self, file_path: str) -> Dict[str, Any]:
        """
        Process NAR markdown file and extract structured data.

        Args:
            file_path: Path to .md file containing NAR form data
        ...
        try:
            ...
            logger.info(f"✅ Successfully processed NAR Page {self.page_number} form")
            return result
    ...
    def _get_all_schema_fields(self) -> Dict[str, Dict[str, Any]]:
        """Get all fields from NAR schema."""
        all_fields = {}
    ...
    def _normalize_field_names(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Only keeps fields that exist in NAR schema.
        """
        normalized = {}
    ...
    def _categorize_into_sections(self, data: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
        """
        Categorize fields into NAR sections using schema.
        Uses typed_data which has been type-converted.
        """
        sections = {}

        # Initialize sections based on page-specific schema
        schema_fields = self._get_all_schema_fields()
        section_names = set()
    ...
    def _validate_against_schema(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Validate data against NAR schema.
        ...
        return {
            ...
            # Required fields documentation metrics
    ...
    def _generate_summary(
        ...
        Maps field keys to descriptions from NAR schema.
        Includes all fields from sections (omits None/null values).
        """
        summary = []
        summary.append(f"=== NAR PAGE {self.page_number} SUMMARY ===\n")

        schema_fields = self._get_all_schema_fields()

        # Get unique sections in the order they appear in schema
        section_order = {}
        for field_def in schema_fields.values():
            if isinstance(field_def, dict):
                section_enum = field_def.get('section')
                if section_enum:
                    if hasattr(section_enum, 'value'):
                        ...
                        section_key = str(section_enum)

                    if section_key not in section_order:
                        section_order[section_key] = section_key.replace('_', ' ').upper()
        ...
        if val_metrics:
            ...
            summary.append(f" Data coverage: {data_coverage:.1f}%")
            summary.append(f" Data completeness for required fields: {data_req_completeness:.1f}%")
            summary.append("\n")
    ...
    def export_individual_json(self, result: Dict[str, Any], output_path: str):
        """Export result sections as JSON."""
        try:
            output = Path(output_path)
            output.parent.mkdir(parents=True, exist_ok=True)
    ...
    def export_text_report(self, result: Dict[str, Any], output_path: str):
        ...
        try:
            ...
            lines = []
            lines.append(f"NAR PAGE {self.page_number} FORM REPORT")
            lines.append(f"{'=' * 80}")
